...
from mg_viz.stats_small import viz_stats_small_3p, viz_stats_small_5p

parser = argparse.ArgumentParser("Visualize stats collected for small set of genomes"
                                 " (e.g. < 20 genomes).")
...
def viz_stats_small(env, df_per_gene, tools, ref_5p, ref_3p):
    ...
    viz_stats_small_3p(env.duplicate({"pd-work": pd_3p}), df_per_gene, tools, ref_3p)
    viz_stats_small_5p(env.duplicate({"pd-work": pd_5p}), df_per_gene, tools, ref_5p)
...
def main(env, args):
    ...
    viz_stats_small(env, df_per_gene, tools, args.ref_5p, args.ref_3p)
